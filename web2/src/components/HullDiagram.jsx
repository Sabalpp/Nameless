...
export default function HullDiagram({
  simResult,
  progress: tickProgress = null,
  activeTick = null,
  routeGeo: routeGeoProp = null,
  onGlobePoint = null,
  freeCamera = false,
  showShip = true,
}) {
  ...
  useEffect(() => {
    ...
    if (freeCamera) {
      cam.up.set(0, 1, 0);
      cam.position.copy(route.midpointUnit).multiplyScalar(GLOBE_RADIUS * 3.1);
      cam.lookAt(GLOBE_CENTER);
    } else {
    ...
    controls.target.copy(freeCamera ? GLOBE_CENTER : shipFocus);
    controls.enableDamping = true;
    controls.dampingFactor = 0.08;
    controls.enablePan = freeCamera;
    controls.minDistance = CAMERA_MIN_DISTANCE;
    controls.maxDistance = CAMERA_MAX_DISTANCE;
    controls.minPolarAngle = freeCamera ? 0 : CAMERA_MIN_POLAR_ANGLE;
    controls.maxPolarAngle = freeCamera ? Math.PI : CAMERA_MAX_POLAR_ANGLE;
    ...
    controls.mouseButtons = {
      ...
      RIGHT: freeCamera ? THREE.MOUSE.PAN : THREE.MOUSE.ROTATE,
    };
    controls.update();
    ...
    const onPointerUp = (event) => {
      const movement = pointerDownPosition
        ? Math.hypot(
      ...
      if (movement <= 5 && onGlobePoint) {
        ...
        if (hit) {
          const unit = hit.point.clone().sub(GLOBE_CENTER).normalize();
          onGlobePoint({
            lat_deg: THREE.MathUtils.radToDeg(Math.asin(clamp(unit.y, -1, 1))),
            lon_deg: THREE.MathUtils.radToDeg(Math.atan2(unit.x, unit.z)),
          });
        }
      }
    };
  ...
  }, [routeGeoKey, onGlobePoint, freeCamera, showShip]);
  ...
  useEffect(() => {
    ...
    if (!freeCamera && controls && cam && shipFocus) {
    ...
    stateRef.current.shipProgress = routeProgress;
  }, [progress, routeGeoKey, freeCamera]);
